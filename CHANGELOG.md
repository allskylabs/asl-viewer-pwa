...
- Timelapse Browser rows now show preview thumbnails when a poster image (.jpg, .webp, or .png) exists next to the MP4 in S3.
- Backend detects poster files from the existing S3 key listing (no extra API calls) and returns a presigned `posterUrl`.
- `posterUrl` field added to `TimelapseListItem` type.
- Clean dark placeholder with duration badge and date when no poster exists.
- Thumbnail images use `loading="lazy"` for performance.
- Thumbnails shrink at ≤700px and hide at ≤480px to keep mobile rows compact.
...
[0.3.1]: https://github.com/allskylabs/asl-viewer-pwa/compare/v0.3.0...v0.3.1
